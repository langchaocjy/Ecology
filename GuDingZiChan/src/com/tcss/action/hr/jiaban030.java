...
public class jiaban030 extends BaseBean implements Action{
	...
	public String execute(RequestInfo requestinfo) {
		...
		if (!arr[0].equals("S")) {
			...
			requestinfo.getRequestManager().setMessagecontent("加班单写入失败："+flag);
		}
		return SUCCESS;
	}
	...
	public String inJiaban030(JSONObject mainData,JSONArray detailData,String workcode,String requestid){
		writeLog("jiaban  mainData:"+mainData);
		writeLog("jiaban  detailData: "+detailData);
    	...
    	try {
			...
			for (int i = 0; i < item_type0s.length; i++) {
				...
				item_type02.setZOANU(mainData.getString("lsh"));//oa单号
				item_type02.setZITEM(String.valueOf(i+1));//oa行号
				item_type02.setPERNR(mainData.getString("sqrgh"));//员工工号
				item_type02.setBEGDA(deleteH(mainData.getString("sqrq")));//开始日期
				item_type02.setLGART(js.getString("jblxmz"));//事项类别
				item_type02.setZPAYH(js.getString("bcjbss"));//时数
				item_type02.setZPAYD(js.getString("ts"));//天数,加班天数在主表
				item_type02.setZYEARMONTH(js.getString("nf")+js.getString("yf"));//考勤月份
				item_type02.setAEDAT(deleteH(mainData.getString("scrq")));//更改日期
				item_type02.setBK1(js.getString("yl1"));//预留字段1
				item_type02.setBK2(js.getString("yl2"));//预留字段2
			...
			if (type.equals("S")) {
				flag = type;
				rs.execute("update formtable_main_217 set xxlxhead='"+type+"' where requestid='"+requestid+"'");
				...
				writeLog("index: "+index+" detail_type0s.length:"+detail_type0s.length);
				for (int i = 0; i < detail_type0s.length-1; i++) {
					...
					rs.execute("update formtable_main_217 set xxlxhead='"+type+"' where requestid='"+requestid+"'");
					rs.execute("update formtable_main_217_dt1 set xxlx='"+type_s+"', xxnr='"+msg_s+"' where id='"+j_item.getString("id")+"'");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
