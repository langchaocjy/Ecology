...
public class WriteInSap extends BaseBean implements Action{
	...
	public String execute(RequestInfo request) {
		    ...
		    try 
		    {
				...
				String sql1 = "select id from FORMTABLE_MAIN_186_dt1 where mainid in(select id from FORMTABLE_MAIN_186 where requestid='"+requestid+"')";    
				rs.execute(sql1);
				String allid = "";
				while(rs.next()) 
				{
				...
				for (int j = 0; j < idarr.length; j++) {
					String sql2 = "update FORMTABLE_MAIN_186_DT1 set mxhh='"+(j+1)+"' where id ='"+idarr[j]+"'";
					writeLog("[sql2--->]"+sql2);
					rs.execute(sql2);
				}
