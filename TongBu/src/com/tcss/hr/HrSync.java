...
public class HrSync extends HrUtil implements HrmSynService {
	...
	public String SynTimingToOAHrmResource() {
		...
		for (int i = 0; i < hrmResources.size(); i++) {
			...
			hrData.updateField(json.getString("员工编号"), json.getString("一级部门"),json.getString("人事范围"),
					json.getString("人事子范围"),json.getString("员工组"),json.getString("员工子组"),
					json.getString("入职日期"),json.getString("工时制"),json.getString("考勤方式"),
					json.getString("排班"));
		}
		try {
...
HrSync hr = new HrSync();
String a = hr.SynTimingToOAHrmResource();
}
}
