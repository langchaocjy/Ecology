...
public class FYBXAction extends BaseBean implements Action{
	...
	private JSONObject executeRcbx(RequestInfo request) {
	    ...
	    if (recordSet.next()) {
			bxr = recordSet.getString("jkr");//报销人
...
//			bizhong = recordSet.getInt("skbb");//币种
	    ...
	    else if (isCreateNode)
	    {
	    	...
	    	if (rs.next()) 
	    	{
	         ...
	         {
	             String fycdbm = recordSet.getString("ssbm");
	             
	             RequestBudgetEntryDTO entry = new RequestBudgetEntryDTO();
	             
	             entry.setExpenseTypeNumber(recordSet.getString("jkkm"));//费用项目编码--费用科目
	    ...
	    else
	    {
	    	...
	    	if (rs.next()) 
	    	{
	         ...
	         sql = "select t2.*,t1.ssgs,t1.ssbm from " + formtable + " t1," + formtable + "_dt1 t2 where requestId=" + requestid + " and t1.id=t2.mainid";
	         recordSet.execute(sql);
	         int seq = 0;
	         while (recordSet.next())
	         {
	           ...
	           entry.setExpenseTypeNumber(recordSet.getString("jkkm"));
	           ...
	           entry.setCompanyNumber(EasService.getSubCode(recordSet.getString("ssgs")));//预算承担公司
	           entry.setDepartmentNumber(EasService.getDeptCode(recordSet.getString("ssbm")));//预算承担部门
	           entry.setRemark(recordSet.getString("zhaiyao"));//摘要
	           entryList.add(entry);
	         }
	         req.setEntryList(entryList);
