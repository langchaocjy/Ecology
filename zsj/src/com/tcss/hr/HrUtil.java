  ...
  extends TcssUtil
{
  ...
  public boolean updateField(String code, String field2,String field3,String field4,String field5,String field6,String field7,
		  String field8,String field9,String field10)//员工编号workcode，一级部门field2
  {
    RecordSet rs = new RecordSet();
    boolean b = false;
    if (!field2.equals(""))
    {
      JSONObject userID = getObjectById(code, "HrmResource", "workcode");
      JSONObject deptID = getObjectById(field2, "hrmdepartment", "departmentcode");
      if(!userID.isEmpty() && !deptID.isEmpty()){
      String sql = "select * from  cus_fielddata where scopeid=-1 and id='" + userID.getString("id") + "'";
      rs.execute(sql);
      if (rs.next())
      {
        sql = "update cus_fielddata set field2='" + deptID.getString("id") + "',field3='"+field3+"',field4='"+field4+"',field5='"+field5+"',field6='"+field6+"',field7='"+field7+"',"
        		+ "field8='"+field8+"',field9='"+field9+"',field10='"+field10+"' where scopeid=-1 and id='" + userID.getString("id") + "'";
        write("更新的自定义字段信息： "+sql);
        b = rs.execute(sql);
      }
      else
      {
        sql = "insert into cus_fielddata(scope,scopeid,id,field2,field3,field4,field5,field6,field7,field8,field9,field10)"
        		+ "values('HrmCustomFieldByInfoType','-1','" + userID.getString("id") + "','" + deptID.getString("id") + "','"+field3+"','"+field4+"','"+field5+"','"+field6+"','"+field7+"',"
  ...
  {
    RecordSet rs = new RecordSet();
    String sql = "update formtable_main_30 set zzmc='" + json.getString("部门名称") + "' ,cbzxmc='" + json.getString("成本中心名称") + "' ,cbfzrmc='" + json.getString("成本负责人名称") + "' "
    		+ ",cbfzrbm='" + json.getString("成本负责人编码") + "' ,frgsmc='" + json.getString("法人公司名称") + "' ,frgsbm='" + json.getString("法人公司编码") + "',"
    	    + "zccbzx='" + json.getString("主次成本中心") + "',cjsx='"+json.getString("层级属性")+"',bedat='"+json.getString("开始日期")+"',endat='"+json.getString("结束日期")+"' "
    	    + " where zzbm='" + json.getString("部门编码") + "' and cbzxbm='" + json.getString("成本中心编码")+"'";
  ...
  public boolean selectHrmjob2(JSONObject json){
	  RecordSet rs = new RecordSet();
	  String sql = "select * from uf_bmgwxx where jobtitlecode='" + json.getString("岗位编码") + "'";
	  rs.execute(sql);
	return rs.next();
  }
  ...
  public boolean updateHrmjob2(JSONObject json){
	  RecordSet rs = new RecordSet();
	    String sql = "update uf_bmgwxx set jobtitlemark='" + json.getString("岗位简称") + "' ,jobtitlename='" + json.getString("岗位名称") + "' ,"
	    		...
	    		+ "bk2='" + json.getString("预留字段2") + "' where jobtitlecode='" + json.getString("岗位编码") + "'";
	    return rs.execute(sql);
  }
