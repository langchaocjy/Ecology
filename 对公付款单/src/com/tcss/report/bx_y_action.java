...
public class bx_y_action extends BaseBean implements Action{
	@Override
	public String execute(RequestInfo request) {
		new BaseBean().writeLog("[---------------------->]   "+request.getRequestManager().getBillTableName());
		try {
		    ...
		    rs.execute("select lcspbm,sjbm from "+formtable+"_dt3 where mainid ="+mainid);
		    writeLog("{msg -------------> sql2:  } "+"select lcspbm,sjbm from "+formtable+"_dt3 where mainid = "+mainid);
		    while (rs.next()) {
